...
import { deleteMainTaskData, getMainTask } from "@/lib/features/mainTask/mainTaskSlice";
...
const TaskPage = ({
  ...
  const filteredActivities = React.useMemo(() => {
    if (mainTaskData?.length > 0) {
      const filtetData = activeFilter === "all" ? mainTaskData : mainTaskData?.filter((item: any) => item?.leadStatus === activeFilter)
      if (searchValue) {
        return dataFilter(
  ...
  }, [mainTaskData, searchValue, activeFilter]);
