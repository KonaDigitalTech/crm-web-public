...
const CustomInput = ({ lableValue, handelOnKey, disabled = false, typeValue, placeholder, onChange, name, value = '', error, accept, mandatory = false }: { error?: string | null, value?: any, name?: String, onChange?: (e: any) => void, lableValue: String, typeValue: String, placeholder?: String, disabled?: boolean, accept?: string, handelOnKey?: (e: any) => void, mandatory?: boolean }) => {
                ...
                {typeValue === "file" ?
                    ...
                    : typeValue === 'date' || typeValue === 'datetime-local' ? <input placeholder={`${placeholder ? placeholder : ""}`}
                        ...
                        <input placeholder={`${placeholder ? placeholder : ""}`}
                            ...
                            disabled={disabled}
                            onChange={onChange}
                            onKeyDown={name === "ask_ai" ? handelOnKey : name === "phone" || name === "feeQuoted" || name === "amountSpent" || name === "fullNumber" || name === "feeAmount" || name === "paidNumber" || name === "totalFees" || name === "feePaid" || name === "dueAmmount" || name === "mobile" || name === 'phoneNumber' || name === 'countryCode' ? handleKeyDown : name === "name" ? handleTextKeyDown : undefined}
                            className="h-full text-lg w-full border-b border-[#0003] -mt-2 bg-transparent pt-4 pb-1.5 font-sans font-semibold text-blue-gray-700 outline outline-0 transition-all placeholder-shown:border-blue-gray-200 focus:border-gray-900 focus:outline-0 " />}
            </div>
        </div>
    )
